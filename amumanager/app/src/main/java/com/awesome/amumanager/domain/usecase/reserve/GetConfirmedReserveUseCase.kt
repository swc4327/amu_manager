...
class GetConfirmedReserveUseCase @Inject constructor(private val repository: ReserveRepository) {
    operator fun invoke(ReserveLists: MutableLiveData<ArrayList<ReserveList>>,
                        storeId: String,
                        lastId: String) {
        repository.getConfirmedReserveList(ReserveLists, storeId, lastId)
    }
}